...
function CheckTables($connection, $config)
{
    ...
    $query = "CREATE TABLE IF NOT EXISTS {$config["base_database"]}.Cities (timemark DATETIME,id INT, name NVARCHAR(100),clan INT);\n";
    echo $query;
    $result = $connection->query($query);
    if (!$result) {
        die("Error during creating palyers table".$connection->connect_errno.$connection->connect_error);
    ...
    if ($result->num_rows <= 0) {
        ...
        foreach ($attacks as $attack) {
            ...
            if ($result->num_rows <= 0) {
                ...
                $attack["declared"]=str_replace("T"," ",$attack["declared"]);
                $attack["declared"]=str_replace("Z"," ",$attack["declared"]);
                $attack["resolved"]=str_replace("T"," ",$attack["resolved"]);
                $attack["resolved"]=str_replace("Z"," ",$attack["resolved"]);
        ...
        foreach ($cities as $city) {
            ...
            if ($city["clan"]==null) {
                ...
                $query = "INSERT INTO {$config["base_database"]}.Cities (timemark,id,name,clan) VALUES (\"$date\",{$city["id"]},\"{$city["name"]}\",{$city["clan"]});\n";
            }
            echo $query;
            $result = $connection->query($query);
            if (!$result) {
